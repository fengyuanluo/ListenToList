...
- 搜索入口只在 `StorageType.OPEN_LIST` 设备页出现
- 默认折叠为标题栏中的搜索图标，点击后再展开搜索框
- 搜索中不破坏原目录浏览状态；退出搜索后返回目录列表
- 点击结果：
  - 目录：进入目录
  - 音频：直接播放
  - 其他文件：定位到所在目录
...
- 搜索结果页顶部只保留返回按钮、搜索框和紧凑筛选按钮
- 搜索框下方为实例标签胶囊，支持点击切换和左右滑动切换实例
- 不同实例改为横向分页，而不是纵向堆叠
- 每个实例单独处理：
  - 有结果正常展示
  - 没结果显示空态
  - 失败显示实例级错误卡片
- 当前实例页滚动接近底部时会自动懒加载下一页
- 切换实例时会保留各自的列表滚动位置
- 点击结果：
  - 目录：直接打开对应目录页
  - 音频：直接播放
